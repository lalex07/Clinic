...
Older session notes moved verbatim out of `progress.md` to keep the working file small. Newest-first. Nothing was deleted — the live working notes (orientation, ~5 most recent sessions, and the evergreen Design reference / Pages-not-yet-built / Constraints sections) remain in `progress.md`.
...
- **規則入帳**：在 `CLAUDE.md` 的「Design rules」節新增一條硬規則「**No eyebrow pill above page headings.**」——page hero／page-head 直接進標題，禁止再於 `<h1>` 上方加那顆圓角點＋文字的 eyebrow 膠囊（舊「● 新店・木柵・興隆・中山」式）；麵包屑與斜體章節 kicker 不在此限，只禁這顆圓角膠囊元件。僅動 `CLAUDE.md`（不 served）。
