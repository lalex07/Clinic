...
The rule has a concrete measured constraint behind it: **the header bar's intrinsic width must fit inside the `.wrap` content column** — `--maxw` 1140 − 2×24px padding = **1092px**. If the bar is wider than that, `立即預約` sits to the right of the gutter every other element aligns to at *every* desktop width (it was 1127.18px, overhanging by 35.17px, and `body { overflow-x: hidden }` hid the symptom), and the header starts clipping far higher up than it should. Measure the intrinsic width — don't eyeball it — and fix an overflow by slimming the bar (gaps, link padding), **not** by raising the collapse breakpoint. The collapsed-nav breakpoint (`assets/site.css`, currently `max-width: 1129px`) is derived from that measurement, not chosen: intrinsic width + 48px gutters, plus a small buffer. It is mirrored by `NAV_COLLAPSE_MQ` in `assets/site.js` and the two must stay in sync. Two other breakpoints deliberately sit at 760px and must NOT be dragged along with it: `.footer__locs`, and the analytics `device` threshold in `site.js` (moving that one would break the continuity of historical `events.device` data).
...
- **Engagement analytics (`public.events`, sent from `assets/site.js`)** — the site's only runtime call to Supabase: a cookieless, fire-and-forget anon INSERT (`pageview` / `booking_click` / `faq_view`). Hard constraints, all of them load-bearing for the privacy claim: **no cookie and no `localStorage`** (a per-visit `sessionStorage` id only), **no IP / UA / name / email column**, `location.pathname` only (never the query string or hash), the call wrapped so nothing it does can throw into page code, and a localhost guard so dev traffic never reaches the production table. Anon may only INSERT — there is deliberately **no UPDATE policy at all**; SELECT is `is_admin()`, DELETE is `is_admin_mfa()`. **Retention is 90 days** (院長's decision, published in `privacy.html`): any purge — manual or a job added later — honours that period, and changing the period means changing the page. Any change to what is collected, or to how long it is kept, must be mirrored in `privacy.html` **in the same commit** — the policy page and `site.js` must never drift.
